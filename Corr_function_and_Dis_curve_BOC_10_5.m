...
%%
%%%相关器间隔
BW=30*1.023e6;d=0.2*Tc;%相关器间隔，BOC(14,2)相关器间隔
...
%%
t_begin=0;
[S_CA_receiver,t_receiver]=yt_BOCs_function(c,t_begin,t_begin+Tp,Tc,fs,f_sample);
h_wait=waitbar(0);
for k=1:N_tao
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [s_E,t_E]=yt_BOCs_function(c,t_begin-tao(k)+d/2,t_begin-tao(k)+Tp+d/2,Tc,fs,f_sample); 
    [s_P,t_P]=yt_BOCs_function(c,t_begin-tao(k),t_begin-tao(k)+Tp,Tc,fs,f_sample);
    [s_L,t_L]=yt_BOCs_function(c,t_begin-tao(k)-d/2,t_begin-tao(k)+Tp-d/2,Tc,fs,f_sample);
    %%
    N_zong=length(t_P);   
...
saveas(gcf,'DIS_BOC_2dian51.fig');