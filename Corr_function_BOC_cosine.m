...
t_end=t_begin+Tp;
t_ref_begin=0;
[s_receive,t_r]=yt_BOCc_function(c,t_begin,t_end,Tc,fs,f_sample);
%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%BOC%%%%%%%%%%%%%
N_zong=length(t_r);
loop=5000;
...
%         V_BOC(n)=IE_BOC^2+QE_BOC^2-IL_BOC^2-QL_BOC^2;
        ...
        %相关函数
        %%
        [s1_P,s2_P,t_P]=wby_BOC_cosine_ref_1_waveform_function(c,t_begin-tao(n),t_end-tao(n),Tc,fs,f_sample);
...
%         R(n) = (R1(n)+abs(R1(n)))*(R2(n)+abs(R2(n)));
          R(n) =R1(n)*R2(n);
%         R(n) = (R1(n)+abs(R1(n)))*(R2(n));
...
close(h_wait);
% %%%%%%%%%%%%%%%
figure;plot(tao/Tc,R,tao/Tc,R1,tao/Tc,R2,'LineWidth',2);xlabel('Code delay [chips]');ylabel('Cross-correlation');grid on;
legend('( R1 + abs(R1) )*( R2 + abs(R2) )','R1','R2');
