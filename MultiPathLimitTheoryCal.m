function [ErrEnvelope, MeanEnvelope]=MultiPathLimitTheoryCal(f,PSD,alpha_dB,del_dist)
% 理论计算多径误差
% f:        频率向量，Hz
% PSD：      归一化功率谱密度， W/Hz
% alpha_dB:  多径直达幅度比   dB
...
for m = 1:length(TaoValues)
    ...
    UpPart1(m)     = alpha * trapz(f, PSD .* sin(2*pi*f*det_tao) .*f);      % 0相位分子
    DownPart1(m)   = 2 * pi * trapz(f, f .* PSD .* (1 + alpha*cos(2*pi*f*det_tao)) .* f);    % 0相位分母
    UpPart2(m)     = -alpha * trapz(f, PSD .* sin(2*pi*f*det_tao) .*f);      % 180相位分子
    DownPart2(m)   = 2 * pi * trapz(f, f .* PSD .* (1 - alpha*cos(2*pi*f*det_tao)) .* f);    % 180相位分母
end
