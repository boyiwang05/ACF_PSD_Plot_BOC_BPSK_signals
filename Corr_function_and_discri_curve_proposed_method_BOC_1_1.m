...
%功能：求鉴别曲线斜率
%信号：BOC(1,1)
%本地参考波形生成函数：wby_BOC_ref_2_waveform_function
%带宽：4*1.023MHz
%（双边）相关器间隔：0.2Tc
%环路带宽： V1:K= 6.9845
%       V2:K=  13.9706
...
%%
%%%相关器间隔
BW=100*1.023e6;d=0.2*Tc;%BOC(10,5)相关器间隔
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
N_BW=10000;
...
for n=1:N_tao
        ...
        [s1_E,s2_E,t_E]=wby_BOC_ref_2_waveform_function(c,t_begin+d/2-tao(n),t_end+d/2-tao(n),Tc,fs,f_sample);
        [s1_L,s2_L,t_L]=wby_BOC_ref_2_waveform_function(c,t_begin-d/2-tao(n),t_end-d/2-tao(n),Tc,fs,f_sample);
...
%         V_BOC(n)=IE_BOC^2+QE_BOC^2-IL_BOC^2-QL_BOC^2;
        ...
        %相关函数
        %%
        [s1_P,s2_P,t_P]=wby_BOC_ref_2_waveform_function(c,t_begin-tao(n),t_end-tao(n),Tc,fs,f_sample);
%         [s_BOC_P,t_BOC_P]=yt_BOCs_function(c,t_begin-tao(n),t_end-tao(n),Tc,fs,f_sample);
