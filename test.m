...
%%
%%%相关器间隔
BW=200*1.023e6;d=0.02*Tc;%BOC(10,5)相关器间隔
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
N_BW=10000;
...
%         
        %% 鉴别器1
        V_MCF(n)  =IE1*IE2+QE1*QE2-IL1*IL2-QL1*QL2;%鉴别器1
        %% 鉴别器2
          V_MCF_mod(n) = IE1*IE2 + abs( IE1*IE2 ) + QE1*QE2 +abs( QE1*QE2 ) - ( IL1*IL2 + abs( IL1*IL2 ) + QL1*QL2 +abs( QL1*QL2 ) );%鉴别器2     
        %%
...
legend('R1*R2','R1','R2');
title('BOC(14,2)')
saveas(gcf,'CCF_MCF_BOC_14_2.fig');

figure;plot(tao/Tc,R1.*R2+abs(R1.*R2),'-',tao/Tc,R1,'-.',tao/Tc,R2,'--','LineWidth',2);xlabel('Code delay [chips]');ylabel('Cross-correlation');grid on;
legend('R1*R2+abs(R1*R2)','R1','R2');
title('BOC(14,2)')
saveas(gcf,'CCF_MCF_mod_BOC_14_2.fig');

figure;
plot(tao/Tc,V_MCF,'LineWidth',2);
xlabel('Code delay [chips]');ylabel('Discriminator output');
% legend('V1','V2','V3');
grid on;
saveas(gcf,'DIS_MCF_BOC_14_2.fig');

figure;
plot(tao/Tc,V_MCF_mod,'LineWidth',2);
xlabel('Code delay [chips]');ylabel('Discriminator output');
% legend('V1','V2','V3');
grid on;
% figure;plot(tao/Tc,V_BOC);
saveas(gcf,'DIS_MCF_mod_BOC_14_2.fig');